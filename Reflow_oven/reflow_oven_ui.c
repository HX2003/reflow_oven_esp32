...
static void lv_ex_settings_2(lv_obj_t * parent);
...
LV_EVENT_CB_DECLARE(abort_confirm_reflow_event_cb);
...
void reflow_oven_ui(void)
{
	...
	door_get_sw_state()?display_door_status(REFLOW_OVEN_DOOR_STATUS_UNLOCKED):display_door_status(REFLOW_OVEN_DOOR_STATUS_LOCKED);
    ...
    enc_drv.read_cb = drv_encoder_read;
	enc_drv.feedback_cb = drv_sound_feedback;
...
void display_heater_status(reflow_oven_heater_status_type_t state){
    if(state == REFLOW_OVEN_HEATER_STATUS_DISABLED){
		...
		lv_obj_set_style_local_image_recolor_opa(heater_state_icon, LV_IMG_PART_MAIN, LV_STATE_DEFAULT, LV_OPA_TRANSP);
		...
		lv_obj_set_style_local_image_recolor_opa(heater_state_icon, LV_IMG_PART_MAIN, LV_STATE_DEFAULT, LV_OPA_100);
        lv_obj_set_style_local_image_recolor(heater_state_icon, LV_IMG_PART_MAIN, LV_STATE_DEFAULT, REFLOW_OVEN_YELLOW);
        lv_obj_align(heater_state_icon, NULL, LV_ALIGN_CENTER, 0, -16);
...
static void start_reflow(){
    ...
    chart_data = lv_chart_create(box, NULL);
    ...
    for(int i=0; i<chart_horizontal_max_points; i++){
	...
	reflow_oven_pid_status_type_t pid_state = REFLOW_OVEN_PID_STATUS_PRE_PRESOAK;
	xQueueSend(pid_queue, &pid_state, portMAX_DELAY);
}
...
static void toggle_door(){
	...
	bool door_state = door_get_sw_state();
	
	
	const char door_opening[] = "Door opening...";
	const char door_closing[] = "Door closing...";
	
    lv_obj_t * title = add_title(!door_state?door_opening:door_closing, popup, REFLOW_OVEN_THEME_TITLE);
	...
	*/
	...
	reflow_oven_door_queue_type_t door_state_v = {door_state, true};
	
	xQueueSend(door_queue, &door_state_v, portMAX_DELAY);
	
    reflow_oven_anim_out(bg_shade, 3000);
    reflow_oven_anim_out(popup, 3000);
}

void reflow_oven_focus_door_icon(){
	if(home_icon.icon1->obj!=NULL&&home_icon.icon2->obj!=NULL){ //Ensure that icons are available
		lv_group_remove_all_objs(g);
...
void reflow_complete(){
	...
	lv_obj_set_event_cb(reflow_bottom_btn, abort_confirm_reflow_event_cb);
}
...
LV_EVENT_CB_DECLARE(abort_reflow_event_cb){
     if (e == LV_EVENT_CLICKED) {
        ...
        lv_obj_set_event_cb(abort_btn, abort_confirm_reflow_event_cb);
...
static icon_t * add_icon(lv_obj_t * parent, const void * src, const void * label_src, lv_color_t color) {
    ...
    lv_obj_set_user_data(img_label, (lv_obj_user_data_t)lv_obj_get_y(img_label));
...
static void lv_ex_settings_2(lv_obj_t * parent)
{
    ...
    i->state = (int32_t)(reflow_oven_setting.pid_p_presoak/REFLOW_OVEN_P_SCALE_FACTOR);
    sprintf(i->value, "%.3f", REFLOW_OVEN_P_SCALE_FACTOR*i->state);

    i = &pid_presoak_menu_items[PID_ITEM_INTEGRAL];
    i->state = (int32_t)(reflow_oven_setting.pid_i_presoak/REFLOW_OVEN_I_SCALE_FACTOR);
    sprintf(i->value, "%.3f", REFLOW_OVEN_I_SCALE_FACTOR*i->state);

    i = &pid_presoak_menu_items[PID_ITEM_DERIVATIVE];
    i->state = (int32_t)(reflow_oven_setting.pid_d_presoak/REFLOW_OVEN_D_SCALE_FACTOR);
    sprintf(i->value, "%.3f", REFLOW_OVEN_D_SCALE_FACTOR*i->state);
    //
    i = &pid_soak_menu_items[PID_ITEM_PROPORTIONAL];
    i->state = (int32_t)(reflow_oven_setting.pid_p_soak/REFLOW_OVEN_P_SCALE_FACTOR);
    sprintf(i->value, "%.3f", REFLOW_OVEN_P_SCALE_FACTOR*i->state);

    i = &pid_soak_menu_items[PID_ITEM_INTEGRAL];
    i->state = (int32_t)(reflow_oven_setting.pid_i_soak/REFLOW_OVEN_I_SCALE_FACTOR);
    sprintf(i->value, "%.3f", REFLOW_OVEN_I_SCALE_FACTOR*i->state);

    i = &pid_soak_menu_items[PID_ITEM_DERIVATIVE];
    i->state = (int32_t)(reflow_oven_setting.pid_d_soak/REFLOW_OVEN_D_SCALE_FACTOR);
    sprintf(i->value, "%.3f", REFLOW_OVEN_D_SCALE_FACTOR*i->state);
    //
    i = &pid_reflow_menu_items[PID_ITEM_PROPORTIONAL];
    i->state = (int32_t)(reflow_oven_setting.pid_p_reflow/REFLOW_OVEN_P_SCALE_FACTOR);
    sprintf(i->value, "%.3f", REFLOW_OVEN_P_SCALE_FACTOR*i->state);

    i = &pid_reflow_menu_items[PID_ITEM_INTEGRAL];
    i->state = (int32_t)(reflow_oven_setting.pid_i_reflow/REFLOW_OVEN_I_SCALE_FACTOR);
    sprintf(i->value, "%.3f", REFLOW_OVEN_I_SCALE_FACTOR*i->state);

    i = &pid_reflow_menu_items[PID_ITEM_DERIVATIVE];
    i->state = (int32_t)(reflow_oven_setting.pid_d_reflow/REFLOW_OVEN_D_SCALE_FACTOR);
    sprintf(i->value, "%.3f", REFLOW_OVEN_D_SCALE_FACTOR*i->state);
    ...
    if(e == LV_EVENT_REFRESH) {
       ...
       for(i = 0; display_and_sound_menu_items[i].type != LV_SETTINGS_TYPE_INV; i++) {
        ...
        if(strcmp("Brightness", act_item->name) == 0) {
			display_brightness_set(act_item->state);
...
static void pid_presoak_menu_event_cb(lv_obj_t * btn, lv_event_t e)
{
    ...
    if(e == LV_EVENT_REFRESH) {
        ...
        if(strcmp("Proportional", act_item->name) == 0) {
            reflow_oven_setting.pid_p_presoak = REFLOW_OVEN_P_SCALE_FACTOR*act_item->state;
            sprintf(act_item->value, "%.3f", REFLOW_OVEN_P_SCALE_FACTOR*act_item->state);
            lv_settings_refr(act_item);
        }
        else if(strcmp("Integral", act_item->name) == 0) {
            reflow_oven_setting.pid_i_presoak = REFLOW_OVEN_I_SCALE_FACTOR*act_item->state;
            sprintf(act_item->value, "%.3f", REFLOW_OVEN_I_SCALE_FACTOR*act_item->state);
            lv_settings_refr(act_item);
        }
        else if(strcmp("Derivative", act_item->name) == 0) {
            reflow_oven_setting.pid_d_presoak = REFLOW_OVEN_D_SCALE_FACTOR*act_item->state;
            sprintf(act_item->value, "%.3f", REFLOW_OVEN_D_SCALE_FACTOR*act_item->state);
            lv_settings_refr(act_item);
        }
    }
}
...
static void pid_soak_menu_event_cb(lv_obj_t * btn, lv_event_t e)
{
    ...
    if(e == LV_EVENT_REFRESH) {
        ...
        if(strcmp("Proportional", act_item->name) == 0) {
            reflow_oven_setting.pid_p_soak = REFLOW_OVEN_P_SCALE_FACTOR*act_item->state;
            sprintf(act_item->value, "%.3f", REFLOW_OVEN_P_SCALE_FACTOR*act_item->state);
            lv_settings_refr(act_item);
        }
        else if(strcmp("Integral", act_item->name) == 0) {
            reflow_oven_setting.pid_i_soak = REFLOW_OVEN_I_SCALE_FACTOR*act_item->state;
            sprintf(act_item->value, "%.3f", REFLOW_OVEN_I_SCALE_FACTOR*act_item->state);
            lv_settings_refr(act_item);
        }
        else if(strcmp("Derivative", act_item->name) == 0) {
            reflow_oven_setting.pid_d_soak = REFLOW_OVEN_D_SCALE_FACTOR*act_item->state;
            sprintf(act_item->value, "%.3f", REFLOW_OVEN_D_SCALE_FACTOR*act_item->state);
            lv_settings_refr(act_item);
        }
    }
...
static void pid_reflow_menu_event_cb(lv_obj_t * btn, lv_event_t e)
{
    ...
    if(e == LV_EVENT_REFRESH) {
        ...
        if(strcmp("Proportional", act_item->name) == 0) {
            reflow_oven_setting.pid_p_reflow = REFLOW_OVEN_P_SCALE_FACTOR*act_item->state;
            sprintf(act_item->value, "%.3f", REFLOW_OVEN_P_SCALE_FACTOR*act_item->state);
            lv_settings_refr(act_item);
        }
        else if(strcmp("Integral", act_item->name) == 0) {
            reflow_oven_setting.pid_i_reflow = REFLOW_OVEN_I_SCALE_FACTOR*act_item->state;
            sprintf(act_item->value, "%.3f", REFLOW_OVEN_I_SCALE_FACTOR*act_item->state);
            lv_settings_refr(act_item);
        }
        else if(strcmp("Derivative", act_item->name) == 0) {
            reflow_oven_setting.pid_d_reflow = REFLOW_OVEN_D_SCALE_FACTOR*act_item->state;
            sprintf(act_item->value, "%.3f", REFLOW_OVEN_D_SCALE_FACTOR*act_item->state);
            lv_settings_refr(act_item);
        }
    }
